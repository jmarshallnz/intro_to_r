...
Today we'll be looking at `group_by` and `summarise` to choose or create new columns, and show how to produce 'small multiple' plots with `ggplot2`.
...
clean %>% summarise(Total = sum(Students), Average = Total/n(), Median = median(Students))
```
...
```{r}
clean %>% summarise(Max = max(Students))
```
...
You can return multiple numbers with each summary function, but you have to then know which each
of them are which. Better to use functions that return a single number if you can.

```{r}
clean %>%
  summarise(Min = min(Students), Max = max(Students))

clean %>%
  summarise(Range = range(Students), Which = c("Min", "Max"))
```
...
```{r}
clean %>% 
  filter(Level == 13, Gender == "Female") %>%
  group_by(School) %>%
  summarise(Total = sum(Students)) %>%
  arrange(desc(Total))
```
Helper function for finding the largest or smallest entries (max/min) is `slice_max()`
...
4. Produce a graph of the total number of students by year level. Hint: First get the data frame you want, and then use `geom_col`.
...
clean %>%
  ...
  mutate(Total = sum(Students)) %>% # Handy way of utilising how the dropping of grouping works with each summarise()
  ggplot() +
  geom_col(aes(x=Level, y=Students, fill=Gender),
           position = "dodge")
```
...
```{r}
eu_year_by_gender <- clean %>%
  filter(EthnicGroup == "European") %>%
  group_by(Level, Gender) %>%
  summarise(Total = sum(Students))
ggplot(eu_year_by_gender) +
  geom_col(mapping=aes(x=Level, y=Total, fill=Gender), position='dodge')

as_year_by_gender <- clean %>%
  filter(EthnicGroup == "Asian") %>%
  group_by(Level, Gender) %>%
  summarise(Total = sum(Students))
...
ggplot(data = ethnicity_by_year_by_gender) +
  geom_col(mapping=aes(x=Level, y=Total, fill=Gender), position='dodge') +
  facet_grid(rows = vars(Gender), cols = vars(EthnicGroup), scales = "free_y") +
  guides(fill = "none")
```
