...
A reminder that the penguin data is available here: https://github.com/allisonhorst/palmerpenguins
...
in 2012 versus 2019 by ethnicity.

---

## Data today

We have the number of 2 year olds immunised in 2012 and 2019 by ethnicity:

```{r, include=FALSE}
library(lubridate)
all_vacc <- read_csv(here::here("data/vaccinations_all.csv")) %>%
  filter(DHB == "National") %>%
...
The response is often referred to as the **dependent** variable - indicating that it's value might depend on the value of the explanatory variable.

The explanatory variables are referred to as being **independent** - it doesn't depend on the response.
...
The division by the standard deviations ensures that correlation is not affected by scale: Multiply $x$ by 5 and you'll get the same correlation. Add 1000 to $y$ and you'll get the same correlation.
...
Small counts (e.g. number of legs) can be considered qualitative.
...
Compare number of 2 year olds immunised in 2012 with those in 2019 by Ethnicity:
...
Thus the reason we isolate for 14 days at the border,
even if a day 3 test returns negative.
]
